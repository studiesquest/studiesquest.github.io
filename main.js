...
searchInput.addEventListener("input", () => {
  ...
  grid.querySelectorAll(".tile").forEach(tile => {
    ...
    tile.style.display = title.includes(filter) ? "" : "none";
  });
});
...
document.querySelectorAll(".filters button").forEach(btn => {
  btn.addEventListener("click", () => {
    ...
    grid.querySelectorAll(".tile").forEach(tile => {
      if(filter === "All") tile.style.display = "";
      else if(filter === "Favorites") tile.style.display = tile.classList.contains("favorite") ? "" : "none";
      else tile.style.display = tile.querySelector(".tag").textContent === filter ? "" : "none";
    });
  });
});
